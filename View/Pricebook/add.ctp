        ...
        <dd>
          <select id="price_book_customer_group_id">
            <?php foreach ($groups as $group) { ?>
              <option
                  value="<?php echo $group['MerchantCustomerGroup']['id']; ?>"><?php echo $group['MerchantCustomerGroup']['name']; ?></option>
            <?php } ?>
          </select>
        </dd>
        <dt>Outlet</dt>
        <dd>
          <select id="price_book_outlet_id">
            ...
            <?php foreach ($outlets as $outlet) { ?>
              <option
                  value="<?php echo $outlet['MerchantOutlet']['id']; ?>"><?php echo $outlet['MerchantOutlet']['name']; ?></option>
            <?php } ?>
          </select>
        </dd>
      </dl>
...
function documentInit() {
  ...
  $(document).on("click", ".data-found", function () {
    $(".price_book_individual tbody").append('<tr class="added_price_book_entry" data-id="' + $(this).attr("data-id") + '"><td>' + $(this).text() + '</td><td class="entry_supply_price">' + $(this).attr("data-supply_price") + '</td><td><input type="text" class="entry_markup" value="' + $(this).attr("data-markup") + '"></td><td><input type="text" class="entry_discount" value="0"></td><td class="entry_retail_price_exclude_tax">' + $(this).attr("data-retail-price") + '</td><td class="entry_tax" tax-rate="' + $(this).attr("data-tax-rate") + '">' + $(this).attr("data-tax") + '</td><td><input type="text" class="entry_retail_price_include_tax" value="' + $(this).attr("data-price") + '"></td><td><input type="number" class="entry_min_unit"></td><td><input type="number" class="entry_max_unit"></td><td><div class="clickable remove"><i class="glyphicon glyphicon-remove"></i></div></td></tr>');
    calculation();
    $(".search_result").hide();
  });
